...
function Movies() {
  ...
  const updateMovieHandler = async (movies) => {
    const response = await api.put(`/movies/${movies.movieID}`, movies);
    const { movieID, title, genre } = response.data;

    setMovies(
      movies.map((movie) => {
        return movie.movieID === movieID ? { ...response.data } : movie;
      })
    );
    window.location.reload(false);
  };
